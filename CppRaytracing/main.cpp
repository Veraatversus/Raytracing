...
HitableList randomSphereScene() {
	...
	world.add(std::make_shared<Sphere>(Vec3{ 4.0f, 1.0f, 0.0f }, 1.0f, std::make_shared<Metal>(Vec3{ 0.7f, 0.6f, 0.f }, 0.0f)));
		...
		for (int y = 0; y < int(imageHeight); ++y) {
			...
			for (uint32_t x{ 0 }; x < imageWidth; ++x) {
				...
				for (uint32_t s{ 0 }; s < samplesPerPixel; ++s) {
					const float u{ (x + Rand::rand01()) / (imageWidth - 1) };
					const float v{ (y + Rand::rand01()) / (imageHeight - 1) };
